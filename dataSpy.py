...
def dataSpy(kw, pg, moreInfomation=False):  # 数据捉虫函数
    ...
    for i in videoResult:  # 处理搜索结果 计算权重
        ...
        __debug = ""  # 初始化 测试变量
        __goodTag = ["鬼畜", "人力VOCALOID", "音MAD", "MMD·3D",
                     "MMD", "MMD.3D", "手书"]  # 初始化列表 大力推荐的标签
        for b in i:           # 遍历  字典
        ...
        #↓↓↓↓ 计算权重
        __weight = int(__weight_random + __weight_click + __weight_sendTime + __weight_author + __weight_danmaku +
                       __weight_collect + __weight_coin + __weight_like + __weight_vertical + __weight_cooperation + __weight_tag)
        #print("权重值：", __weight)
        # 在视频结果列表中插入权重
        __debug = ""
        i.update({"__weight": __weight, "__filter": __filter,
                 "__authorExclude": __authorExclude, "debug": __debug})
